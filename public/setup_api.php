...
if ($action === 'init_database') {
    ...
    try {
        ...
        $tRecordOwners = table_ident($schema, 'spw_record_owners');
        $tImports = table_ident($schema, 'spw_imports');
        $tImportRowsLog = table_ident($schema, 'spw_import_rows_log');
        $tRelMigrations = table_ident($schema, 'spw_release_migrations');
        $tRagFiles = table_ident($schema, 'spw_rag_files');
        $tRagQueries = table_ident($schema, 'spw_rag_queries');
        $tRagChunks = table_ident($schema, 'spw_rag_chunks');
        $tRagQuerySources = table_ident($schema, 'spw_rag_query_sources');
        $tAutomationRuns = table_ident($schema, 'spw_automation_runs');
        $tAutomationEmails = table_ident($schema, 'spw_automation_emails');
        $tAnonLog = table_ident($schema, 'spw_anonymization_log');
        $tAnonReport = table_ident($schema, 'spw_anonymization_report');
        ...
        $queries = [
            ...
            // spw_migrations: bootstrap tracker kept in sync with includes/admin/migrations.php
