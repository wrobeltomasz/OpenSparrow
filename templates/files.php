...
<main>
    <section id="filesSection">
                    ...
                    <tr>
                        <?php if ($canEdit) : ?>
                            <th class="th-select">
                                <input type="checkbox" class="select-all-cb"
                                       aria-label="<?=
                                           htmlspecialchars($filesLabels['selectAll'], ENT_QUOTES, 'UTF-8')
                                        ?>"
                                       title="<?=
                                           htmlspecialchars($filesLabels['selectAllToggle'], ENT_QUOTES, 'UTF-8')
                                        ?>">
                            </th>
                        <?php endif; ?>
                        <?php foreach ($fileColumns as $fileColumn) : ?>
                            <th data-sort="<?= htmlspecialchars($fileColumn['sort'], ENT_QUOTES, 'UTF-8') ?>"
                                ...
                                <span class="th-label th-tip"><?=
                                    htmlspecialchars($fileColumn['label'], ENT_QUOTES, 'UTF-8')
                                ?></span>
                            </th>
                        <?php endforeach; ?>
        ...
        <div id="filesActions" class="actions">
            <div class="left">
                <button id="btnRefreshFiles"><?=
                    htmlspecialchars($filesLabels['refresh'], ENT_QUOTES, 'UTF-8')
                ?></button>
            </div>
        ...
        <div id="fileUploadBar" class="f-upload-bar">
            ...
            <select id="fileRelatedId" class="f-input f-input-w220" disabled>
                <option value=""><?=
                    htmlspecialchars($filesLabels['optSelectTableFirst'], ENT_QUOTES, 'UTF-8')
                ?></option>
            </select>
            <button id="btnUpload" class="success"><?=
                htmlspecialchars($filesLabels['upload'], ENT_QUOTES, 'UTF-8')
            ?></button>
